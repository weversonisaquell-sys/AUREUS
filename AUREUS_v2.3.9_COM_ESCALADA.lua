...
local function stopWallClimb()
    ...
    if root then
        local force = root:FindFirstChild("AUREUS_WallClimbForce")
        if force then
            force:Destroy()
        end
    end
...
local function startWallClimb()
    ...
    wallClimbConnection = RunService.Heartbeat:Connect(function()
        ...
        -- Procura uma parede na direção para onde o personagem está olhando.
        local result = workspace:Raycast(
            root.Position,
            root.CFrame.LookVector * 3.2,
            wallRayParams
        )

        local climbingWall = result
            and result.Instance
            and result.Instance.CanCollide
            and math.abs(result.Normal.Y) < 0.45

        if climbingWall and humanoid.MoveDirection.Magnitude > 0.05 then
            -- Mantém o personagem encostado na parede e sobe enquanto caminha contra ela.
            ...
            if not force then
                ...
                force.Name = "AUREUS_WallClimbForce"
            ...
            -- Pequena força em direção à parede para não cair para trás,
            -- combinada com velocidade vertical para a escalada.
            local towardWall = -result.Normal * 3
            ...
            WallStatus.Text = "Parede detectada: escalando."
            ...
            if WallStatus.Text == "Parede detectada: escalando." then
                WallStatus.Text = "Modo de escalada ativo. Encoste e caminhe contra uma parede."
                WallStatus.TextColor3 = Colors.Success
            end
        end
    end)
