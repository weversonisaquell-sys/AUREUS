...
local function startWallClimb()
    ...
    wallClimbConnection = RunService.RenderStepped:Connect(function()
        ...
        for _, result in ipairs(hits) do
            if result and result.Instance and result.Instance.CanCollide and math.abs(result.Normal.Y) < 0.5 then
                hit = result
                break
            end
        end
        ...
        if hit then
            ...
            velocity.VectorVelocity = Vector3.new(push.X, wallClimbSpeed, push.Z)
            humanoid:ChangeState(Enum.HumanoidStateType.Freefall)
            WallStatus.Text = "Parede detectada: subindo."
            WallStatus.TextColor3 = Colors.Success
        else
            if velocity then velocity:Destroy() end
            if WallStatus.Text == "Parede detectada: subindo." then
                WallStatus.Text = "Escalada ativa. Encoste em uma parede para subir."
                WallStatus.TextColor3 = Colors.Success
            end
        end
    end)
...
WallToggle.MouseButton1Click:Connect(function()
    ...
    if wallModeEnabled then
        ...
        WallToggle.Text = "DESLIGAR ESCALADA"
        WallStatus.Text = "Modo de escalada ativo. Aproxime-se de uma parede e caminhe contra ela."
        WallStatus.TextColor3 = Colors.Success
        startWallClimb()
...
-- Seleção e criação de escada local na parede escolhida.
WallMiniPanel.Size = UDim2.new(0, 275, 0, 220)
WallToggle.Position = UDim2.new(0, 14, 1, -58)

local LadderButton = Instance.new("TextButton")
LadderButton.Position = UDim2.new(0,14,1,-108)
LadderButton.Size = UDim2.new(1,-28,0,38)
LadderButton.BackgroundColor3 = Color3.fromRGB(48,50,60)
LadderButton.BorderSizePixel = 0
LadderButton.Text = "SELECIONAR PAREDE E CRIAR ESCADA"
LadderButton.TextColor3 = Colors.Text
LadderButton.TextSize = 10
LadderButton.Font = Enum.Font.GothamBold
LadderButton.ZIndex = 31
LadderButton.Parent = WallMiniPanel
local lc = Instance.new("UICorner"); lc.CornerRadius = UDim.new(0,7); lc.Parent = LadderButton

local selectingWall = false
local ladderFolder = workspace:FindFirstChild("AUREUS_LocalLadders") or Instance.new("Folder")
ladderFolder.Name = "AUREUS_LocalLadders"
ladderFolder.Parent = workspace

local function createLadder(hit)
    if not hit or not hit.Instance or not hit.Instance:IsA("BasePart") or math.abs(hit.Normal.Y) > 0.5 then
        WallStatus.Text = "Selecione uma parede vertical."
        return
    end

    local old = ladderFolder:FindFirstChild("AUREUS_Ladder")
    if old then old:Destroy() end

    local model = Instance.new("Model")
    model.Name = "AUREUS_Ladder"
    model.Parent = ladderFolder

    local normal = hit.Normal
    local point = hit.Position + normal * 0.55
    local height, width = 18, 5
    local horizontal = Vector3.new(normal.Z, 0, -normal.X).Unit

    for _, side in ipairs({-width/2, width/2}) do
        local rail = Instance.new("Part")
        rail.Anchored = true
        rail.CanCollide = true
        rail.Size = Vector3.new(0.35, height, 0.35)
        rail.CFrame = CFrame.new(point + horizontal * side + Vector3.new(0,height/2,0))
        rail.Parent = model
    end

    for y = 1.2, height - 0.5, 1.4 do
        local rung = Instance.new("Part")
        rung.Anchored = true
        rung.CanCollide = true
        rung.Size = Vector3.new(width,0.3,0.55)
        rung.CFrame = CFrame.lookAt(point + Vector3.new(0,y,0), point + normal)
        rung.Parent = model
    end

    WallStatus.Text = "Escada criada na parede selecionada."
    WallStatus.TextColor3 = Colors.Success
end

LadderButton.MouseButton1Click:Connect(function()
    selectingWall = not selectingWall
    LadderButton.Text = selectingWall and "TOQUE/CLIQUE EM UMA PAREDE..." or "SELECIONAR PAREDE E CRIAR ESCADA"
    WallStatus.Text = selectingWall and "Seleção ativa. Clique em uma parede." or "Seleção cancelada."
end)

UserInputService.InputBegan:Connect(function(input, processed)
    if not selectingWall or processed then return end
    ...
    params.FilterDescendantsInstances = {Player.Character, ladderFolder}
