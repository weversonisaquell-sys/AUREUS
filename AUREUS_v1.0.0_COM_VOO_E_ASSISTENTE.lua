...
local UpButton = CreateButton(FlyCard, "↑ SUBIR", UDim2.new(0,18,0,84), UDim2.new(0.5,-24,0,42))
local DownButton = CreateButton(FlyCard, "↓ DESCER", UDim2.new(0.5,6,0,84), UDim2.new(0.5,-24,0,42))
...
FlyToggle.MouseButton1Click:Connect(function()
    if flying then
        stopFly(); UpButton.Visible=false; DownButton.Visible=false
        FlyToggle.Text="ATIVAR VOO"; FlyStatus.Text="Voo desligado."
    else
        startFly(); UpButton.Visible=true; DownButton.Visible=true
        FlyToggle.Text="DESLIGAR VOO"; FlyStatus.Text="Voo ativado. Segure SUBIR ou DESCER."; FlyStatus.TextColor3=Colors.Success
    end
end)

local AssistantCard = CreateCard(LocalPlayerPage, 235)
CreateLabel(AssistantCard, "AUREUS ASSISTANT", UDim2.new(0,18,0,12), UDim2.new(1,-36,0,28), Colors.Red, 17)
local QuestionBox = Instance.new("TextBox")
QuestionBox.Position=UDim2.new(0,18,0,48); QuestionBox.Size=UDim2.new(1,-36,0,42)
QuestionBox.BackgroundColor3=Color3.fromRGB(22,24,31); QuestionBox.BorderSizePixel=0
QuestionBox.PlaceholderText="Pergunte sobre o painel..."; QuestionBox.PlaceholderColor3=Colors.SubText
QuestionBox.TextColor3=Colors.Text; QuestionBox.TextSize=13; QuestionBox.Font=Enum.Font.Gotham
QuestionBox.ClearTextOnFocus=false; QuestionBox.Parent=AssistantCard
local qc=Instance.new("UICorner"); qc.CornerRadius=UDim.new(0,6); qc.Parent=QuestionBox
local AnswerLabel=CreateLabel(AssistantCard,"Pergunte o que uma aba ou botão faz.",UDim2.new(0,18,0,105),UDim2.new(1,-36,0,48),Colors.SubText,12)
AnswerLabel.TextWrapped=true; AnswerLabel.TextYAlignment=Enum.TextYAlignment.Top
local AskButton=CreateButton(AssistantCard,"PERGUNTAR",UDim2.new(0,18,0,168),UDim2.new(1,-36,0,42))
AskButton.MouseButton1Click:Connect(function()
    local q=string.lower(QuestionBox.Text or "")
    if q:find("voo") or q:find("voar") then AnswerLabel.Text="O voo mostra os botões SUBIR e DESCER enquanto está ativado."
    elseif q:find("executor") or q:find("loadstring") then AnswerLabel.Text="A aba Executor reúne as ferramentas configuradas para execução no projeto."
    elseif q:find("martelo") or q:find("gear") then AnswerLabel.Text="A aba Martelo reúne funções como reentrar no servidor e ações locais."
    elseif q:find("amigo") then AnswerLabel.Text="A aba Amigos permite selecionar jogadores pelo nome usando a lógica autorizada pelo servidor."
    else AnswerLabel.Text="AUREUS Assistant pode explicar as abas e funções disponíveis no painel." end
