...
CreateLabel(ExecutorCard, "LUA / LOADSTRING EXECUTOR", UDim2.new(0,18,0,12), UDim2.new(1,-36,0,30), Colors.Red, 17)
...
CodeBox.PlaceholderText = 'Cole código Lua ou loadstring(game:HttpGet("URL"))()'
CodeBox.Text = ""
...
local RunButton = CreateButton(ExecutorCard, "EXECUTAR LUA", UDim2.new(0,18,0,212), UDim2.new(1,-36,0,42))
...
RunButton.MouseButton1Click:Connect(function()
	...
	local Success, Result = pcall(function()
		-- Aceita tanto Lua comum colado diretamente quanto chamadas loadstring.
		local Compiler = loadstring or load
		if type(Compiler) ~= "function" then
			error("Este ambiente não possui um compilador Lua disponível.")
		end

		local Function, CompileError = Compiler(Code)
		if not Function then
			error("Erro de sintaxe Lua: " .. tostring(CompileError))
		end
...
local function makeLuaScript(request)
    local r = string.lower(request or "")

    if r:find("veloc") or r:find("speed") then
...
print("Velocidade definida para 50")]]
    elseif r:find("pulo") or r:find("jump") then
        return [[local Players = game:GetService("Players")
local player = Players.LocalPlayer
local character = player.Character or player.CharacterAdded:Wait()
local humanoid = character:WaitForChild("Humanoid")
...
humanoid.UseJumpPower = true
humanoid.JumpPower = 44
print("Pulo definido para 44")]]
    elseif r:find("teleport") or r:find("teleporte") then
        return [[local Players = game:GetService("Players")
local player = Players.LocalPlayer
...
mouse.Button1Down:Connect(function()
    ...
    elseif r:find("botão") or r:find("button") or r:find("gui") or r:find("painel") then
        return [[local Players = game:GetService("Players")
local player = Players.LocalPlayer
...
button.MouseButton1Click:Connect(function()
    ...
    elseif r:find("voar") or r:find("voo") then
        return [[local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
RunService.RenderStepped:Connect(function()
    ...
    if root then
        ...
        return [[-- Script Lua criado pela AUREUS AI
-- Descreva mais detalhes para eu adaptar o comportamento.

local function main()
    print("Olá! Este é o seu novo script Lua.")
    -- Adicione aqui a lógica solicitada.
end
...
local function answerQuestion(question)
    ...
    local wantsScript =
        q:find("crie") or q:find("criar") or q:find("faça") or
        q:find("faca") or q:find("gere") or q:find("gerar") or
        q:find("script lua") or q:find("código lua") or q:find("codigo lua")

    if wantsScript then
        return "Claro! Aqui está um script Lua baseado no seu pedido:\n\n" .. makeLuaScript(question)
    elseif q:find("^oi") or q:find("olá") or q:find("ola") then
        return "Oi, " .. Player.Name .. "! Eu sou a AUREUS AI. Posso conversar normalmente, explicar o painel e criar exemplos de scripts Lua."
    elseif q:find("quem é você") or q:find("o que você é") or q:find("quem e voce") or q:find("o que e voce") then
        return "Eu sou a AUREUS AI, a assistente do painel AUREUS. Posso responder perguntas sobre o painel e gerar exemplos de código Lua a partir do que você pedir."
    elseif q:find("tudo bem") or q:find("como você está") or q:find("como voce esta") then
        return "Tudo bem! E com você? Estou pronta para conversar ou ajudar a montar um script Lua."
    elseif q:find("obrigad") then
        return "De nada! Se quiser, peça um novo script Lua ou pergunte qualquer coisa."
    elseif q:find("voo") or q:find("voar") then
    ...
    elseif q:find("executor") or q:find("loadstring") or q:find("lua") then
        return "O Executor aceita código Lua colado diretamente e também código que use loadstring quando o ambiente disponibilizar essa função."
    elseif q:find("martelo") or q:find("gear") then
        return "A aba Gear reúne funções e ferramentas organizadas no painel."
    elseif q:find("amigo") then
        return "A aba Amigos reúne as opções relacionadas aos jogadores disponíveis no jogo."
    elseif q:find("script") or q:find("painel") or q:find("aureus") then
        ...
        return "Posso conversar normalmente e também criar exemplos de scripts Lua. Por exemplo: \"crie um script Lua de velocidade\" ou \"Oi, o que você é?\""
    end
end
